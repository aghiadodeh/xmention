package com.aghiadodeh.xmention.ui.shadow
...
class ShadowView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null, defStyleInt: Int = 0) : ViewGroup(context, attributeSet, defStyleInt) {
    ...
    init {
        ...
        shadowColor = a.getColor(
            R.styleable.ShadowView_shadowColor, ContextCompat.getColor(context, R.color.shadow_view_default_shadow_color)
        )
        foregroundColor = a.getColor(
            R.styleable.ShadowView_foregroundColor, ContextCompat.getColor(context, R.color.shadow_view_foreground_color_dark)
        )
        ...
        if (child.visibility != View.GONE) {
            ...
            maxWidth =
                if (shadowMeasureWidthMatchParent)
                    ...
                    maxWidth.coerceAtLeast(child.measuredWidth + shadowMarginLeft + shadowMarginRight + lp.leftMargin + lp.rightMargin)
            maxHeight =
                if (shadowMeasureHeightMatchParent)
                    maxHeight.coerceAtLeast(child.measuredHeight + lp.topMargin + lp.bottomMargin)
                else
                    maxHeight.coerceAtLeast(child.measuredHeight + shadowMarginTop + shadowMarginBottom + lp.topMargin + lp.bottomMargin)
    ...
    override fun draw(canvas: Canvas?) {
       try {
           super.draw(canvas)
       } catch (e: IllegalArgumentException) { // java.lang.IllegalArgumentException: Software rendering doesn't support hardware bitmaps
           e.printStackTrace()
       }
